# Attestly Compliance Pre-Mappings

This directory contains Attestly-to-regulation pre-mapping documents. Each document maps Attestly's attestation features to specific clauses of one regulation, with explicit *necessary-but-not-sufficient* flags so the reader can see where Attestly supplies cryptographic evidence and where the institution must layer in additional controls. These documents are decision-support for compliance and engineering leads — **they are not legal advice**, and they are written to be read alongside the working prototype rather than as standalone interpretations.
...
- **HIPAA** — *planned* (GTM Phase 2 or later). The healthcare vertical equivalent: Attestly attestations as supporting evidence for the audit-control and integrity standards of the Security Rule (45 CFR §164.312(b) and §164.312(c)(1)).
