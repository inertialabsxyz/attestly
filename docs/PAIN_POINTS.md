...
A running log of friction encountered while building the Attestly prototype. Each
...
again at the top of `crates/attestly-agents/src/worker.rs:1-21`. AutoAgents
...
**Concrete example:** `crates/attestly-agents/src/worker.rs` and
...
**Concrete example:** `crates/attestly-agents/src/tools.rs` defines two
...
**Phase 4 stretch-task pain.** `crates/attestly-core/src/task.rs:23-34`
...
**Concrete example:**
`crates/attestly-agents/src/parallel.rs` ships its own `ParallelExecution`
...
Phase 2 of Attestly overall should generalise the coordination type
...
and `data/attestly.db` SQLite (Phase 3) are all live in the current tree.
...
trigger (`crates/attestly-agents/src/batcher.rs:193-196`). This is a
...
(`crates/attestly-agents/src/{worker,verifier}.rs`) whose surface mirrors what an
...
(`crates/attestly-agents/src/worker.rs:107-131`), not inside `calculate()`. Three
reasons:
...
Phase 1's Verifier (`crates/attestly-agents/src/verifier.rs:73-123`) computes
...
lives in `attestly-core`.* `TaskExecution` and `ExecutionStatus` are defined in
`crates/attestly-core/src/task.rs` (not in `attestly-agents`) so any consumer — the
...
(`crates/attestly-core/src/execution.rs`).
...
`attestly-core`, file IO in `attestly-core::execution`, lifecycle ownership in
...
take an `attestly-agents` dependency just to deserialise an execution.
...
`worker_attestation: &Attestation` (`crates/attestly-agents/src/verifier.rs`)
...
(`crates/attestly-agents/src/batcher.rs`) checks the count condition
...
on-chain anchoring (Phase 2 of Attestly overall) introduces gas-cost
...
2. **Regeneration costs the full leaf set.** rs_merkle needs every leaf
   ...
   (a plausible Phase 2-of-Attestly scale) is not.
...
1. **Slow Worker / Verifier produces an attestation after its
   ...
   collected (`crates/attestly-agents/src/dispatcher.rs`), and a stage
...
2. **Producer and Batcher are decoupled (e.g. across an HTTP boundary, a
   future Phase 2-of-Attestly feature).** Here a late attestation can arrive
...
of Attestly overall introduces a producer-side retry path that needs
...
`crates/attestly-agents/src/parallel.rs` runs N verifiers concurrently with
