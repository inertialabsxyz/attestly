...
A reading of the prototype's Phase 4 deliverables (`docs/DECISIONS.md`, `docs/PAIN_POINTS.md`, and the "Phase 1 Postmortem" section appended to `planning/awp-prototype-plan.md`), with a recommended next move.
...
- **Code:** ~5,500 LOC across `awp-core` + `awp-agents` + examples. 24 implementation commits.
- **Tests:** 79 passing (46 in `awp-core`, 33 in `awp-agents`) + 9 integration tests.
...
4. **Three storage models for one prototype.** `attestations.json` (Phase 1) + `executions.json` (Phase 2) + `awp.db` SQLite (Phase 3) all live simultaneously, with overlapping semantics. Fine for a prototype, untenable for production.
...
- **`awp-core` is already framework-agnostic.** A swap touches `awp-agents` only — the plan's "Minimal framework coupling in awp-core" risk-mitigation row was honoured.
...
3. **Storage consolidation.** Three models is fine for prototype, untenable for production. Pick one before Phase 2-of-AWP work compounds the divergence.
