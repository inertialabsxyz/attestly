...
The fastest path from "I read about Attestly" to a green check next to a
...
[`https://attestly.xyz/quickstart`](https://attestly.xyz/quickstart)
...
```bash
curl -X POST https://attestly.xyz/v1/account/signup \
...
```json
{
  ...
  "plan": "team",
  "api_key": "9f7e...-...",
  "dashboard_url": "https://app.attestly.xyz/dashboard?welcome=1"
}
```

Free Team-tier trial — 1M attestations per month, no credit card. See
the [pricing page](https://attestly.xyz/#pricing) for what kicks in
above the floor.
...
The package is pure-Python on top of `attestly-core-py` (the PyO3 bindings
...
Set `ATTESTLY_API_KEY` to the key from step 1, then run your agent. Every
node execution emits a signed attestation to the cloud.

!!! note "No LangGraph yet?"
    The OSS [`tools/audit-viewer/`](https://github.com/inertialabsxyz/attestly/tree/main/tools/audit-viewer)
...
Open the dashboard at `https://app.attestly.xyz/dashboard`. The usage
...
2. On each node completion, it hashed the input state and the output
   ...
   and POSTed the result to `https://attestly.xyz/v1/attestations`.
