...
``awp.langgraph.attest`` so every node execution emits a signed
attestation.
...
Optional flags::
    ...
    --sink cloud      Switch to CloudSink. Needs AWP_API_KEY set and an
                      awp-cloud instance reachable at AWP_CLOUD_ENDPOINT
...
try:
    from awp.langgraph import CloudSink, FileSink, attest  # noqa: E402
except ImportError as exc:  # pragma: no cover - friendly error
    print(
        f"error: could not import the awp-langgraph SDK ({exc}).\n"
        "       Install awp-core-py (`maturin develop` in crates/awp-python/) "
        "and langgraph first.",
        file=sys.stderr,
    )
    raise SystemExit(1)
...
def screen_node(state: KycState) -> dict:
    """Mirror ``KycWorker::decide`` from ``crates/awp-agents/src/kyc_agents.rs``."""
...
def _resolve_sink(arg: str):
    if arg == "cloud":
        ...
        if not api_key:
            print("error: --sink cloud requires AWP_API_KEY to be set", file=sys.stderr)
            raise SystemExit(2)
        endpoint = os.environ.get("AWP_CLOUD_ENDPOINT", "http://localhost:8080")
        return CloudSink(api_key=api_key, endpoint=endpoint)
    return FileSink(_REPO_ROOT / "data" / "attestations.jsonl")


def main() -> int:
    parser = argparse.ArgumentParser(description="AWP LangGraph KYC demo")
    parser.add_argument(
        "--sink",
        choices=["file", "cloud"],
        default="file",
