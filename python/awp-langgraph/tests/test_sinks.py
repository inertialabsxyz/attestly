...
# package shared with the SDK, so the top-level `import awp` may not expose
# these symbols depending on path order. See sinks.py for the full rationale.
from awp._native import AgentIdentity, Attestation, sign_attestation
...
def test_file_sink_round_trips_canonical_wire_format(tmp_path) -> None:
    ...
    awp-core-py — the same path the static audit viewer uses.
    """
    path = tmp_path / "att.jsonl"
    sink = FileSink(path)
    ...
    # All the wire fields documented in services/awp-cloud/API.md must be present.
    for field in (
        "id",
        "agent_id",
        "agent_pubkey",
...
def test_cloud_sink_default_endpoint_matches_api_doc() -> None:
    """Documents the canonical hosted endpoint matches services/awp-cloud/API.md."""
    assert CloudSink.DEFAULT_ENDPOINT == "https://app.awp-cloud.xyz"
