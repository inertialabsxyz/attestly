...
Signing is delegated entirely to ``attestly-core-py``: we build the payload dict
described in ``planning/gtm-phase-2-agent-prompts.md`` § Step 3, hand it to
``attestly.sign_attestation``, and the canonical-JSON-and-Ed25519 contract is
...
# Import from attestly-core-py's C-extension submodule rather than the top-level
# `attestly` package: `attestly` is a PEP 420 namespace package shared with this SDK,
# so `import attestly` may execute either package's `__init__.py`. The `_native`
# submodule is unambiguous regardless of import order.
try:
    from attestly import _native as _attestly_core  # type: ignore
except ImportError as exc:  # pragma: no cover - attestly-core-py is a hard dep
    raise ImportError(
        "attestly-langgraph requires attestly-core-py to be installed. "
        "Install it with `pip install attestly-core-py` (or `maturin develop` "
        "from `crates/attestly-python/` for local builds)."
    ) from exc

from attestly.langgraph import telemetry as _telemetry
from attestly.langgraph.sinks import FileSink, Sink

_log = logging.getLogger("attestly.langgraph")

# Marker stashed on a wrapped graph so a second attest() call is a no-op.
_ATTEST_MARKER_ATTR = "__attestly_attest_wrapped__"
...
def attest(
    ...
    *,
    ...
    identity:
        Explicit ``attestly.AgentIdentity``. Overrides the file-store lookup.
    sink:
    ...
    verifier_identity:
        Explicit Verifier ``attestly.AgentIdentity``. Overrides the file-store
        lookup.
    identities_dir:
    ...
    worker = identity or _attestly_core.AgentIdentity.load_or_create(str(ids_dir), agent_id)
    verifier = None
    if verifier_agent_id is not None:
        verifier = verifier_identity or _attestly_core.AgentIdentity.load_or_create(
    ...
    setattr(graph, "_attestly_state", state)
    return graph
...
def _wrap_existing_node(name: str, node: Any, state: _AttestationState) -> Optional[Any]:
    ...
    _log.warning(
        "attestly.langgraph: could not find callable inside node %r — "
...
def _emit_for_node(
    ...
    Both attestations are signed via ``attestly.sign_attestation``, which owns
    ...
    worker_att = _attestly_core.sign_attestation(
    ...
    try:
        attestation_valid = bool(
            _attestly_core.verify_attestation(worker_att, worker_att.agent_pubkey)
        )
    ...
    verifier_payload = {
        ...
        # attestly-core-py v0.1 only emits Completed; the audit trail records
        # the verdict via these two keys instead.
        "references": worker_att.id,
        "attestation_valid": attestation_valid,
        "answer_correct": outputs_agree,
    }
    verifier_att = _attestly_core.sign_attestation(
    ...
    if not outputs_agree:
        _log.warning(
            "attestly.langgraph: Worker/Verifier disagreement on node=%s "
