"""Anonymous SDK telemetry for ``attestly-langgraph``.
...
* Daily aggregate POST to ``https://telemetry.attestly.xyz/v1/usage``.
...
* Opt out via the ``ATTESTLY_TELEMETRY=0`` environment variable or by calling
  :func:`telemetry_disable` from Python.
* The install UUID lives at ``~/.config/attestly/install_id`` (override with
  the ``ATTESTLY_INSTALL_ID_PATH`` environment variable, e.g. for tests).
...
DEFAULT_TELEMETRY_URL = "https://telemetry.attestly.xyz/v1/usage"
TELEMETRY_ENV_VAR = "ATTESTLY_TELEMETRY"
TELEMETRY_URL_ENV_VAR = "ATTESTLY_TELEMETRY_URL"
INSTALL_ID_PATH_ENV_VAR = "ATTESTLY_INSTALL_ID_PATH"
SDK_VERSION = "0.1.0a1"
...
def _default_install_id_path() -> Path:
    ...
    ``$XDG_CONFIG_HOME/attestly/install_id`` if ``XDG_CONFIG_HOME`` is set,
    otherwise ``~/.config/attestly/install_id``. Override with the
    ``ATTESTLY_INSTALL_ID_PATH`` environment variable.
    """
    ...
    return base / "attestly" / "install_id"


def is_enabled() -> bool:
    """True iff telemetry is enabled in the current process.

    Opt-out is honoured both via the ``ATTESTLY_TELEMETRY=0`` environment
...
def telemetry_disable() -> None:
    ...
    Equivalent to ``ATTESTLY_TELEMETRY=0`` but settable from Python. Useful in
...
def telemetry_enable() -> None:
    ...
    Has no effect if ``ATTESTLY_TELEMETRY=0`` (or equivalent) is set in the
...
def _post(url: str, payload: dict, timeout: float = _REQUEST_TIMEOUT_SECONDS) -> bool:
    ...
    req = urllib_request.Request(
        ...
        headers={
            ...
            "user-agent": f"attestly-langgraph/{SDK_VERSION}",
        },
    )
    try:
...
def _start_background_scheduler() -> None:
    ...
    t = threading.Thread(target=_loop, name="attestly-telemetry", daemon=True)
    t.start()
