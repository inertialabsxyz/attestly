...
- :class:`CloudSink` — ``POST /v1/attestations`` to a hosted ``attestly-cloud``
  per ``services/attestly-cloud/API.md``. Retries on 5xx and connection errors;
...
takes one ``attestly.Attestation`` and returns ``None``).

The cloud-side wire contract is locked at ``services/attestly-cloud/API.md`` and
...
# Import the Attestation type from attestly-core-py's C-extension submodule.
# We import from `attestly._native` rather than the top-level `attestly` package
# because `attestly` is a PEP 420 namespace package shared with this SDK — when
# both packages are on the path, `import attestly` may execute either package's
# `__init__.py`, and only `attestly-core-py`'s exposes `Attestation` at the top
# level. The `_native` submodule is unambiguous regardless of import order.
try:
    from attestly._native import Attestation  # type: ignore
except ImportError as exc:  # pragma: no cover - the package depends on attestly-core-py
    raise ImportError(
...
class FileSink:
    ...
    Each line is ``Attestation.to_json()`` from ``attestly-core-py`` — byte-
...
class CloudSink:
    """POST attestations to a hosted ``attestly-cloud`` endpoint.

    Wire contract: ``services/attestly-cloud/API.md`` § ``POST /v1/attestations``.
    ...
    DEFAULT_ENDPOINT = "https://app.attestly.xyz"
