...
One-line LangGraph integration for the **Attestly** —
every node execution emits a cryptographically signed attestation to a
configurable sink (local file, Attestly Cloud, or your own callable).
...
with `python python/attestly-langgraph/examples/kyc_graph.py`.
...
1. Loads (or creates and persists) a stable Ed25519 identity under
   `./data/identities/<agent_id>.json` via `attestly-core-py`'s
...
2. Wraps every node callable so that, after the node returns, the SDK
   ...
   via `attestly.sign_attestation`, and emits the resulting attestation to the
   configured sink.
...
All cryptography is delegated to `attestly-core-py`; the SDK never
...
```python
from attestly.langgraph import FileSink, CloudSink, CallableSink

# 1. JSONL on disk (default if you don't pass `sink=`)
attest(g, agent_id="a", sink=FileSink("data/attestations.jsonl"))

# 2. POST to attestly-cloud (services/attestly-cloud/API.md)
attest(g, agent_id="a", sink=CloudSink(api_key="...", endpoint="https://app.attestly.xyz"))
...
* **`FileSink(path)`** — append JSONL to a local file. Default when no
  sink is passed; lets a team adopt Attestly with zero hosted dependency.
* **`CloudSink(api_key, endpoint)`** — POSTs to Attestly Cloud per
  [`services/attestly-cloud/API.md`](../../services/attestly-cloud/API.md). Retries
...
```python
import attestly
ident = attestly.AgentIdentity.load_or_create("./secrets", "my-agent-01")
...
Signing is delegated to `attestly-core-py` (Ed25519 in Rust). The Phase 2
...
Run `pytest python/attestly-langgraph/tests/bench_signing.py -v -s` to
remeasure on your hardware; paste the printed number back here whenever
the signing path changes.

## Troubleshooting

**"My attestations aren't appearing in attestly-cloud."** Most likely a
...
`https://telemetry.attestly.xyz/v1/usage` on each run.** The payload is
exactly:
...
`~/.config/attestly/install_id` — it is not derived from any account or user
information.
...
GTM Phase 2 of the Attestly project. This release ships the **Step 3** SDK —
`attest()`, the three sinks, dual-agent mode — on top of the Step 5
telemetry scaffold. Cryptography is delegated to the Step 1 `attestly-core-py`
bindings; the hosted `CloudSink` target is the Step 2 `attestly-cloud` service.

## Documentation

Full docs (concepts, integration reference, self-hosted path,
compliance pointers, migration from `FileSink` to `CloudSink`) at
[`docs.attestly.xyz`](https://docs.attestly.xyz). Source under
