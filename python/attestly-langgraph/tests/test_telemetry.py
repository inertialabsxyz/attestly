"""Tests for the ``attestly.langgraph.telemetry`` opt-out and wire contract.

The two load-bearing claims under test:

1. ``ATTESTLY_TELEMETRY=0`` produces **no network calls** (the Step 5 exit
   criterion).
...
def install_id_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setenv("ATTESTLY_INSTALL_ID_PATH", str(target))
    return target


@pytest.fixture
def enabled(monkeypatch: pytest.MonkeyPatch):
    """Drop any stale ATTESTLY_TELEMETRY=0 from the inherited environment."""
    monkeypatch.delenv("ATTESTLY_TELEMETRY", raising=False)
    from attestly.langgraph import telemetry as t
...
def test_env_opt_out_disables_telemetry(
    ...
    """``ATTESTLY_TELEMETRY=0`` must produce no network call (Step 5 exit criterion)."""
    monkeypatch.setenv("ATTESTLY_TELEMETRY", "0")
    from attestly.langgraph import telemetry as t
    ...
    assert sent is False, "no POST should have been attempted under ATTESTLY_TELEMETRY=0"
...
def test_emit_posts_expected_payload(
    ...
    monkeypatch.setenv("ATTESTLY_TELEMETRY_URL", url)
...
def test_default_install_id_path_respects_xdg(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    monkeypatch.delenv("ATTESTLY_INSTALL_ID_PATH", raising=False)
    monkeypatch.setenv("XDG_CONFIG_HOME", str(tmp_path))
    from attestly.langgraph import telemetry as t

    assert t._default_install_id_path() == tmp_path / "attestly" / "install_id"
...
def test_default_install_id_path_falls_back_to_home(
    ...
    assert t._default_install_id_path() == tmp_path / ".config" / "attestly" / "install_id"
...
def test_env_var_overrides_in_process_enable(
    ...
    """``ATTESTLY_TELEMETRY=0`` must beat a subsequent ``telemetry_enable()`` call."""
    monkeypatch.setenv("ATTESTLY_TELEMETRY", "0")
    from attestly.langgraph import telemetry as t
