...
Hosted ingest, search, and share-link service for AWP attestations. The paid
...
1. **Server never holds private keys.** `POST /v1/attestations` accepts a
   ...
   [`awp-core::Attestation::signing_payload`](../../crates/awp-core/src/attestation.rs)
...
SHA=$(...)
docker compose exec awp-cloud /bin/sh -c "echo garbage > /var/lib/awp-cloud/blobs/${SHA:0:2}/${SHA:2:2}/$SHA"
...
```bash
flyctl launch --no-deploy --copy-config --name awp-cloud-staging
flyctl secrets set DATABASE_URL=postgresql://...   # Neon connection string
flyctl secrets set AWP_CLOUD_BASE_URL=https://awp-cloud-staging.fly.dev
flyctl deploy --config fly.toml
flyctl status      # https://awp-cloud-staging.fly.dev/healthz must return 200
```

The DNS for the production `app.awp-cloud.xyz` cutover is **not** an agent
