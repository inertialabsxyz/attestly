...
use awp_cloud::store::{AttestationIndex, Plan};
use awp_cloud::stripe::webhook;
...
async fn dashboard_html_renders() {
    ...
    assert_eq!(status, StatusCode::OK);
    assert!(text.contains("AWP Cloud — Dashboard"));
...
async fn export_then_audit_viewer_can_re_render() {
    ...
    for line in &lines {
        ...
        // Re-verify against the embedded pubkey using awp-core directly —
        // this is exactly what the audit viewer's JS does, just in Rust.
        assert!(
            awp_core::signing::verify_attestation_signature(&att),
...
async fn retention_sweep_deletes_rows_older_than_retention_days() {
    ...
    for i in 0..fresh_count {
        ...
        let canon = awp_cloud::canonical::canonical_blob_bytes(&att);
        let sha = hex::encode(awp_cloud::canonical::blob_sha256(&canon));
    ...
    let (visited, deleted) = awp_cloud::sweeper::sweep_once(Arc::clone(&h.state.db))
        .await
        .unwrap();
    assert!(visited >= 1);
    assert_eq!(
...
async fn retention_sweep_respects_enterprise_horizon() {
    ...
    h.state
        ...
        .await
        .unwrap();
    let (_, deleted) = awp_cloud::sweeper::sweep_once(Arc::clone(&h.state.db))
        .await
        .unwrap();
    assert_eq!(
        deleted, 0,
