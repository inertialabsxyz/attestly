//! `attestly-cloud-seed` — synthetic data generator for hand-testing.
//!
...
//!   `docker compose exec attestly-cloud /usr/local/bin/sweeper run-once` in
//!   the spec.
//!
//! Requires `DATABASE_URL` and (for the default mode) `BLOB_ROOT` to be set
//! the same way `attestly-cloud` itself does.
...
async fn seed_default(db: PgDb, blob_root: PathBuf) -> anyhow::Result<()> {
    ...
    println!("# Attestly cloud seed");
    println!("# account_id : {}", account.id);
    println!("export ATTESTLY_API_KEY={cleartext}");
    ...
    for i in 0..SEED_COUNT {
        ...
        let task_hash = attestly_core::signing::sha256(format!("task-{i}").as_bytes());
...
async fn seed_overage(db: PgDb) -> anyhow::Result<()> {
    ...
    let at = Utc::now();
    println!("# Attestly cloud seed (overage)");
    println!("# account_id : {}", account.id);
    println!("export ATTESTLY_API_KEY={cleartext}");
    println!(
...
async fn seed_old_attestations(db: PgDb, blob_root: PathBuf) -> anyhow::Result<()> {
    ...
    db.create_api_key(account.id, "retention-project", &phc)
        .await?;
    println!("# Attestly cloud seed (old-attestations)");
    println!("# account_id : {}", account.id);
    println!("export ATTESTLY_API_KEY={cleartext}");
    ...
    for i in 0..STALE_COUNT {
        ...
        let task_hash = attestly_core::signing::sha256(format!("stale-task-{i}").as_bytes());
        let output = serde_json::json!({
            "customer_id": customer_id,
            "decision": "Approve",
        })
    ...
    eprintln!(
        "# done. Run `docker compose exec attestly-cloud /usr/local/bin/sweeper run-once` to verify."
    );
    Ok(())
}
