...
//!
//! All verification flows through `attestly-core::Attestation::signing_payload`
//! and `attestly-core::verify_attestation_signature` — there is exactly one
...
//! client and `attestly-core`, not between the client and this service.

use attestly_core::{verify_attestation_signature, Attestation};
...
/// `attestly-core::verify_attestation_signature` so the call site reads
...
mod tests {
    use super::*;
    use attestly_core::{signing::AgentKeypair, AttestationStatus};

    fn fixture_signed() -> Attestation {
        let kp = AgentKeypair::generate();
        let task_hash = attestly_core::signing::sha256(b"task-canonical");
