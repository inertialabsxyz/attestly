# `attestly-cloud` HTTP API — locked contract (v1)

This document is the **load-bearing contract** between `attestly-cloud` (this
...
`crates/attestly-core/src/attestation.rs` — the contract is byte-identical to the
...
```json
{
  ...
  "url":        "https://app.attestly.xyz/share/<token>",
...
`Attestation::signing_payload` in `crates/attestly-core/src/attestation.rs`. Clients
...
`crates/attestly-python/` (Step 1) both reproduce this exactly; cross-language
