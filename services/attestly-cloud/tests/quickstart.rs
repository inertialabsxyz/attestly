...
async fn signup_key_ingests_and_dashboard_polling_sees_it() {
    ...
    let kp = attestly_core::signing::AgentKeypair::generate();
...
async fn quickstart_page_renders_with_required_copy() {
    ...
    assert!(text.contains("pip install attestly-langgraph"));
    assert!(text.contains("/v1/account/signup"));
    assert!(text.contains("from attestly.langgraph import attest"));
