...
After completing a feature or bug fix — before opening a PR — the implementation agent **must** spawn a review agent to verify the work against the source requirements. This is a hard gate.
...
Agent({
  ...
  prompt: """
You are a review agent. Your job is to verify a completed implementation
...
## Requirements source
Read the requirements from: planning/plan.md, section "<Phase N — Section Name>"

## Files changed
<list the changed src files, e.g. src/artifacts.rs, src/types.rs>

## Your task
1. Read the requirements section in planning/plan.md
