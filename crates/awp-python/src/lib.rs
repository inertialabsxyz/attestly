//! PyO3 bindings for `awp-core` — sign and verify AWP attestations from Python.
//!
//! The signing path delegates byte-for-byte to `awp-core` so that a Python
...
use awp_core::attestation::{Attestation as CoreAttestation, AttestationStatus};
use awp_core::identity::{AgentIdentity as CoreAgentIdentity, FileIdentityStore};
use awp_core::signing::{sha256, verify_attestation_signature};
use pyo3::exceptions::{PyIOError, PyTypeError, PyValueError};
use pyo3::prelude::*;
use pyo3::types::{PyAny, PyBool, PyBytes, PyDict, PyFloat, PyInt, PyList, PyString, PyTuple};

/// Canonicalise a Python dict into the byte sequence that AWP signs over.
///
...
fn py_to_json_value(obj: &Bound<'_, PyAny>) -> PyResult<serde_json::Value> {
    ...
    Err(PyTypeError::new_err(format!(
        "unsupported type in AWP payload: {}",
        obj.get_type().name()?
    )))
}

/// Python-visible wrapper around `awp_core::AgentIdentity`.
#[pyclass(module = "awp._native", name = "AgentIdentity", skip_from_py_object)]
#[derive(Clone)]
...
impl PyAgentIdentity {
    ...
    /// generate and save a new one. Matches `awp_core::AgentIdentity::load_or_create`
    ...
    fn from_secret_bytes(agent_id: &str, secret: &Bound<'_, PyBytes>) -> PyResult<Self> {
        ...
        let keypair = awp_core::signing::AgentKeypair::from_secret_bytes(&seed, None)
...
/// Python-visible wrapper around `awp_core::Attestation`. Field access goes
...
#[pyclass(module = "awp._native", name = "Attestation", skip_from_py_object)]
#[derive(Clone)]
...
impl PyAttestation {
    ...
    /// `awp_core::append_attestation` writes to disk.
...
/// `awp_core::signing::tests::cross_keypair_signature_fails_verification`.
#[pyfunction]
...
/// PyO3 module entry point. Exposed to Python as `awp._native` and
/// re-exported by `python/awp/__init__.py`.
#[pymodule]
