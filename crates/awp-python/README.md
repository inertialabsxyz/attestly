...
Python bindings for the [Agent Work Protocol (AWP)](https://github.com/inertialabsxyz/awp)
...
GTM Phase 2, Step 1 of the AWP project. Currently published to **TestPyPI
...
```bash
pip install --index-url https://test.pypi.org/simple/ awp-core-py
```
...
cd crates/awp-python
maturin develop --release
```

`maturin develop` compiles the Rust extension and installs it into the
active venv. The Python import name is `awp`; the distribution name on
PyPI is `awp-core-py`.
...
identity = awp.AgentIdentity.from_secret_bytes("agent-x", b"\x07" * 32)
att = awp.sign_attestation({"k": "v"}, identity, timestamp=1700000000)

# Write the attestation to disk in the same JSON-lines wire format that
# `awp_core::append_attestation` uses.
with open("/tmp/att.json", "w") as f:
    f.write(att.to_json() + "\n")
```

```bash
# Rust side: verify the file using the bundled `awp-verify` binary.
cargo run -p awp-core --bin awp-verify --release < /tmp/att.json
# → ok
```
...
- **Single-process only.** No coordination primitives. Use the LangGraph
  SDK (Step 3) or `awp-cloud` (Step 2) for multi-process attestation
  coordination.
...
Apache-2.0 OR MIT, same as the rest of the AWP repository.
