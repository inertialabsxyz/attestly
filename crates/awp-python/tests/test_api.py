"""Python-side API smoke tests for awp-core-py.
...
def test_generate_yields_32_byte_pubkey() -> None:
    identity = awp.AgentIdentity.generate("agent-gen")
...
def test_load_or_create_persists_across_calls(tmp_path: Path) -> None:
    first = awp.AgentIdentity.load_or_create(str(tmp_path), "agent-persist")
    second = awp.AgentIdentity.load_or_create(str(tmp_path), "agent-persist")
    assert first.public_key == second.public_key

    # Different agent_id ⇒ different key under the same store.
    other = awp.AgentIdentity.load_or_create(str(tmp_path), "agent-other")
    assert other.public_key != first.public_key


def test_load_or_create_writes_file_with_expected_shape(tmp_path: Path) -> None:
    identity = awp.AgentIdentity.load_or_create(str(tmp_path), "agent-shape")
...
def test_sign_then_verify_round_trip() -> None:
    identity = awp.AgentIdentity.generate("agent-roundtrip")
    att = awp.sign_attestation({"task": "hello"}, identity)
    assert awp.verify_attestation(att, identity.public_key) is True


def test_attestation_field_accessors() -> None:
    identity = awp.AgentIdentity.generate("agent-fields")
    att = awp.sign_attestation({"task": "hello"}, identity, timestamp=1700000000)
...
def test_verify_rejects_wrong_pubkey() -> None:
    identity = awp.AgentIdentity.generate("agent-a")
    other = awp.AgentIdentity.generate("agent-b")
    att = awp.sign_attestation({"task": "hello"}, identity)
    assert awp.verify_attestation(att, other.public_key) is False


def test_verify_rejects_short_pubkey() -> None:
    identity = awp.AgentIdentity.generate("agent-short")
    att = awp.sign_attestation({"task": "hello"}, identity)
    with pytest.raises(ValueError, match="pubkey must be 32 bytes"):
        awp.verify_attestation(att, b"\x00" * 8)


def test_payload_must_have_string_keys() -> None:
    identity = awp.AgentIdentity.generate("agent-typed")
    with pytest.raises(TypeError, match="dict keys must be strings"):
        awp.sign_attestation({1: "v"}, identity)


def test_payload_rejects_unsupported_types() -> None:
    identity = awp.AgentIdentity.generate("agent-typed")
    with pytest.raises(TypeError, match="unsupported type"):
        awp.sign_attestation({"k": object()}, identity)


def test_payload_supports_nested_structures() -> None:
    identity = awp.AgentIdentity.generate("agent-nested")
    att = awp.sign_attestation(
...
def test_to_json_round_trips() -> None:
    identity = awp.AgentIdentity.generate("agent-json")
    att = awp.sign_attestation({"task": "hello"}, identity, timestamp=42)
    blob = att.to_json()
...
def test_version_string_is_exposed() -> None:
    assert isinstance(awp.__version__, str)
    assert awp.__version__.count(".") >= 1
