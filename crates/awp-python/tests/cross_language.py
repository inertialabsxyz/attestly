...
# `crates/awp-core/tests/cross_language_vector.rs`. Any intentional
...
def _awp_verify_binary() -> Path:
    """Locate the `awp-verify` binary built from `awp-core`.

    Honours `AWP_VERIFY_BIN` for CI overrides, otherwise searches the
    standard cargo target directories. The binary is built by `make
    check-python` before pytest runs.
    """
    env_override = os.environ.get("AWP_VERIFY_BIN")
    if env_override:
        return Path(env_override)

    # Prefer release if both exist (CI uses release), otherwise dev.
    for profile in ("release", "debug"):
        candidate = REPO_ROOT / "target" / profile / "awp-verify"
        if candidate.exists():
            return candidate

    # Fall back to PATH lookup (e.g. installed via `cargo install`).
    on_path = shutil.which("awp-verify")
    if on_path:
        return Path(on_path)

    raise FileNotFoundError(
        "awp-verify binary not found. Build it with "
        "`cargo build -p awp-core --bin awp-verify` first."
    )


def _build_vector_identity() -> awp.AgentIdentity:
    secret = bytes.fromhex(VECTOR_SECRET_HEX)
    return awp.AgentIdentity.from_secret_bytes(VECTOR_AGENT_ID, secret)


def _build_vector_attestation() -> awp.Attestation:
    identity = _build_vector_identity()
    return awp.sign_attestation(
        VECTOR_PAYLOAD,
        identity,
...
def test_canonical_bytes_match() -> None:
    ...
    actual = awp.canonical_payload_bytes(VECTOR_PAYLOAD)
...
def test_canonical_bytes_insertion_order_independent() -> None:
    ...
    assert awp.canonical_payload_bytes(reordered) == EXPECTED_CANONICAL_PAYLOAD
...
def test_vector_attestation_self_verifies() -> None:
    ...
    att = _build_vector_attestation()
    assert awp.verify_attestation(att, att.agent_pubkey) is True


def test_sign_python_verify_rust() -> None:
    """Python-signed → Rust-verified: pipe JSON through `awp-verify`."""
    try:
        binary = _awp_verify_binary()
    ...
    assert proc.returncode == 0, (
        f"awp-verify rejected a Python-signed attestation. "
...
def test_sign_rust_verify_python() -> None:
    ...
    assert proc.returncode == 0, (
        f"awp-verify --print-canonical failed. "
        f"stderr={proc.stderr.decode(errors='replace')}"
    )
...
def test_python_canonical_payload_matches_attestation_output() -> None:
    ...
    """
    att = _build_vector_attestation()
    canonical = awp.canonical_payload_bytes(VECTOR_PAYLOAD)
