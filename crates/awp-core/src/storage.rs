//! SQLite persistence for AWP attestations, batches, and inclusion proofs.
//!
...
/// Owns a SQLite [`Connection`] and applies the AWP schema on first use.
pub struct Storage {
    conn: Connection,
}

impl Storage {
    /// Open (or create) a SQLite database at `path`, applying the AWP
...
mod tests {
    ...
    fn opens_database_at_filesystem_path() {
        ...
        let path = dir.path().join("nested").join("awp.db");
