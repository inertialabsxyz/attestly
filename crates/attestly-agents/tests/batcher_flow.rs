...
use attestly_agents::{Batcher, BatcherConfig};
use attestly_core::{
    sha256, verify_inclusion, AgentKeypair, Attestation, AttestationStatus, Storage,
};
...
async fn count_trigger_persists_a_batch_with_verifiable_proofs() {
    ...
    let db_path = dir.path().join("attestly.db");
    ...
    for a in &attestations {
        ...
        let leaf = attestly_core::attestation_leaf_hash(&stored_att);
        assert!(
