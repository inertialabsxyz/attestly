...
//!
//! This test pins the exact bytes and signature that the Attestly canonical
//! encoding produces for a fixed payload, identity seed, attestation UUID,
//! and timestamp. The Python-side test at
//! `crates/attestly-python/tests/cross_language.py` asserts the **same** byte
...
use attestly_core::attestation::{Attestation, AttestationStatus};
use attestly_core::signing::{sha256, verify_attestation_signature, AgentKeypair};
use uuid::Uuid;
...
/// The canonical bytes that `attestly.canonical_payload_bytes(payload)` must
...
fn vector_signing_payload_is_pinned() {
    ...
    assert_eq!(
        ...
        "signing_payload hex drifted.\nIf this change is intentional, update EXPECTED_SIGNING_PAYLOAD_HEX in both this file and crates/attestly-python/tests/cross_language.py."
    );
}
...
/// fixed vector. Run with `cargo test -p attestly-core --test cross_language_vector -- print_vector --nocapture --ignored`
