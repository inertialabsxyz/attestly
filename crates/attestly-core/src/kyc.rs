...
//! `kyc_receipts` demo.
//!
//! These live in `attestly-core` so the demo's domain types are framework-agnostic
//! and can be parsed by any consumer (CLI, audit viewer, future SDKs) without
//! pulling in `attestly-agents`. The decision rule itself lives in
//! `attestly-agents::tools::kyc_decide` since that's where the Worker and Verifier
//! invoke it.
//!
//! ## Disclaimer
//!
