...
``attestly`` is shared with the ``attestly-langgraph`` distribution, which adds the
``attestly.langgraph`` subpackage. Extending ``__path__`` here lets that
subpackage be discovered even though this distribution owns ``attestly`` as a
