...
GTM Phase 1 ([`gtm-phase-1-plan.md`](gtm-phase-1-plan.md)) is complete: audit viewer, KYC receipts demo, persistent agent identity, and SR 11-7 pre-mapping are merged to `main`. Phase 2 converts that OSS-credibility surface into a paid funnel by shipping the LangGraph SDK (the adoption wedge) and `attestly-cloud` MVP (the paid surface). The framing for *why* this sequence — OSS-core / paid-hosted, LangGraph as the regulated-buyer concentration point — lives in [`../attestly-market-research.md`](../attestly-market-research.md) and the Context section of `gtm-phase-2-plan.md`.
...
```
Week 1-2 (parallel):  Step 1 — PyO3 Bindings           Step 2 — attestly-cloud Scaffold
                              │                                │
                              └────────────┬───────────────────┘
                                      merge to main
                                           │
...
- **Step 1 and Step 2 may run in parallel** — Step 1 is Rust + PyO3 in a new `crates/attestly-python` crate; Step 2 is a new hosted service in `services/attestly-cloud/`. They share no files. Use separate git worktrees.
- **Both Step 1 and Step 2 must be merged to `main`** before Step 3 starts. Step 3 (the LangGraph SDK) consumes the PyO3 bindings for signing; Step 4 (Stripe billing) attaches to the `attestly-cloud` service.
- **Step 3 and Step 4 may run in parallel** — Step 3 is Python in `python/attestly-langgraph/`; Step 4 is Rust + HTML in `services/attestly-cloud/` and `tools/landing-page/`. The only shared surface is the `CloudSink` ↔ `POST /v1/attestations` contract, which is locked by Step 2's exit criteria; both agents code to that contract independently.
...
- **`attestly-cloud` production cutover.** Step 2's staging URL is the agent's exit bar. Pointing the real `app.attestly.xyz` DNS, configuring secrets, and rotating the production Stripe key are operator tasks.
...
You are implementing Step 1 of GTM Phase 2 of Attestly. The full specification is in `planning/gtm-phase-2-plan.md` under "Step 1 — PyO3 Bindings". Read that section carefully before writing any code.

### Context

The repository contains a complete Rust prototype: `attestly-core` (attestations, signing, Merkle batching, SQLite, persistent agent identity) and `attestly-agents` (Worker, Verifier, Dispatcher, KycWorker, KycVerifier, Batcher). GTM Phase 1 added the static audit viewer (`tools/audit-viewer/`) and the `kyc_receipts` example.

The canonical signing payload lives in `crates/attestly-core/src/attestation.rs::Attestation::signing_payload`. The audit viewer's JS already replicates it byte-for-byte. Your job is to expose the same signing path to Python via PyO3 with the same byte-identical guarantee — so that Python and Rust callers produce signatures that verify across both languages.

This step is **Rust + Python only.** Step 2 (`attestly-cloud` MVP scaffold) ships in parallel in `services/attestly-cloud/` — no merge collisions.

### Your Task

1. **Create `crates/attestly-python/`** as a new workspace member containing a PyO3-bound crate. Add to the workspace `Cargo.toml`. Use `pyo3` with `abi3-py39` so a single wheel covers Python 3.9+ where possible. Module name: `attestly_core_py` (exposed to Python as `attestly`).

2. **Expose the signing API surface:**
   ```python
   attestly.sign_attestation(payload: dict, identity: AgentIdentity) -> Attestation
   attestly.verify_attestation(attestation: Attestation, pubkey: bytes) -> bool
   attestly.AgentIdentity.load_or_create(path: str, agent_id: str) -> AgentIdentity
   attestly.AgentIdentity.generate(agent_id: str) -> AgentIdentity
   ```
...
3. **Byte-identical signatures across Rust and Python.** This is the load-bearing exit criterion. Ship a cross-language self-test:
   - A small Python script in `crates/attestly-python/tests/cross_language.py` that loads a fixed payload from `tests/fixtures/payload.json`, signs in Python, asserts the resulting signature verifies in Rust (via a tiny Rust verifier binary `crates/attestly-python/tests/verify_bin.rs` or a `cargo test`-driven harness — your call), and vice versa.
...
6. **Publish to TestPyPI** under the distribution name `attestly-core-py` (Python import name remains `attestly`). The publish step is gated by a manual workflow_dispatch in CI — don't publish on every push. Real PyPI publish is deferred to Step 6's design-partner gate; document this in the README.

7. **`crates/attestly-python/README.md`** documenting:
   - Install: `pip install --index-url https://test.pypi.org/simple/ attestly-core-py`
...
8. **Tests:**
   ...
   - Python-side tests in `crates/attestly-python/tests/test_api.py` using `pytest` covering identity load/save round-trip and signing roundtrip within Python

### Stubs to pre-place

This step touches `Cargo.toml` (workspace members list). Step 2 also lands a new workspace member (`services/attestly-cloud/` — but it is a separate `Cargo.toml` outside the existing workspace, since it ships independently). **Confirm with Step 2's branch before merge that the workspace `Cargo.toml` does not collide.** Recommended: keep `services/attestly-cloud/` out of the root workspace; declare it as its own workspace. This eliminates the merge conflict.

### Do Not Touch

- `services/attestly-cloud/` — Step 2's domain, does not exist yet
- `python/attestly-langgraph/` — Step 3's domain, does not exist yet
- `tools/landing-page/` — Step 4's domain for pricing-page edits; Step 1 has no reason to touch it
- Existing `crates/attestly-core/` and `crates/attestly-agents/` public APIs — additive only; do not break Rust callers. The four original examples and the `kyc_receipts` example must keep building unchanged.
- `Attestation::signing_payload` in `crates/attestly-core/src/attestation.rs` — your Python canonical encoding must match this, not the other way around. If they disagree, Rust is the source of truth.
...
3. Open a draft PR per `.claude/pull-requests.md` (target `main`, title `feat(python): gtm phase 2 pyo3 bindings for attestly-core`).
...
cd crates/attestly-python && maturin develop
# → wheel built and installed into the active venv

# Cross-language byte-identical test
python -m pytest crates/attestly-python/tests/cross_language.py -v
...
python -c "
import attestly
ident = attestly.AgentIdentity.generate('agent-test')
att = attestly.sign_attestation({'task': 'hello'}, ident)
assert attestly.verify_attestation(att, ident.public_key)
print('ok')
"
# → "ok"
...
## Step 2 — GTM Phase 2: `attestly-cloud` MVP Scaffold

**Branch:** `gtm-phase-2/attestly-cloud-scaffold`

**Prompt:**

You are implementing Step 2 of GTM Phase 2 of Attestly. The full specification is in `planning/gtm-phase-2-plan.md` under "Step 2 — `attestly-cloud` MVP Scaffold". Read that section carefully before writing any code.
...
Your job is to build the hosted-service surface that converts that local flow into a paid product. Procurement-conservative buyers (Persona A, compliance lead Sarah) must be able to use Attestly entirely offline via the OSS path; Persona B (platform operator Marcus) needs the hosted retention, sharing, and search to justify a paid pilot. The hosted service is the **paid wedge** — its existence makes Step 4's Stripe billing possible.

This step is **service code only.** Step 1 (PyO3 bindings) ships in parallel in `crates/attestly-python/` — no merge collisions, since `services/attestly-cloud/` is a separate workspace.

### Your Task

1. **Create `services/attestly-cloud/`** as a new directory with its own `Cargo.toml` (own workspace, not part of the root workspace — this avoids workspace-member collisions with Step 1 and lets the service ship on its own release cadence). Stack:
   - **API:** Rust + Axum
...
2. **HTTP API surface** (versioned under `/v1/`):
   ...
   - `POST /v1/share-links` — body `{"attestation_ids": [...]}` or `{"filter": {...}}`. Returns a tokenised URL `https://app.attestly.xyz/share/{token}`. Tokens are scoped, time-limited (default 30 days, configurable), and revocable.
...
3. **Signature handling rules (load-bearing):**
   ...
   - Re-use the existing canonical-bytes / signing-payload logic from `crates/attestly-core` — depend on it as a workspace path (`{ path = "../../crates/attestly-core" }`) so there is exactly one canonical encoder in the repo.

4. **Web viewer** at `services/attestly-cloud/web/` — server-rendered (or single-page bundled into the binary) replica of the static audit viewer (Phase 1 Step 1), with server-backed search, share-link generation, and an account dashboard. **Reuse the existing static viewer's in-browser JS verification logic** — copy or vendor it; the viewer must independently re-verify, even on a server-backed page, so a compromised server cannot silently serve unverifiable receipts. Placeholder domain in code: `https://app.attestly.xyz`.

5. **Local dev:** `docker compose up` (from `services/attestly-cloud/`) brings up Postgres, Minio, and the API server. A `make seed` target inserts 10k synthetic attestations for testing search and pagination.
...
6. **Account model (for Step 4 to attach billing to):**
   ...
   Use migrations via `sqlx-cli`. Migrations live in `services/attestly-cloud/migrations/`.

7. **Deployment:** Provide a `Dockerfile`, `fly.toml` (or Railway equivalent), and a `services/attestly-cloud/README.md` documenting the deployment model, the key-handling story (server never holds private keys), the data model, and current limitations (single region US-East, no SLA, no HA).

### Stubs to pre-place

For Step 3 (LangGraph SDK) and Step 4 (pricing/billing), both of which depend on this service, pre-place:

1. **A locked HTTP contract document** at `services/attestly-cloud/API.md` describing the `POST /v1/attestations` request and response shapes — Step 3's `CloudSink` codes against this contract.
2. **A Stripe webhook stub** at `services/attestly-cloud/src/billing.rs` containing an empty handler `pub async fn handle_stripe_webhook(...)` that returns 501. Step 4 fills it in; this prevents merge conflicts on the routing layer.
...
- `crates/attestly-python/` — Step 1's domain
- `python/attestly-langgraph/` — Step 3's domain, does not exist yet
- `tools/landing-page/pricing.html` — Step 4's domain
- Existing `crates/attestly-core/` and `crates/attestly-agents/` public APIs — depend on them but do not modify them
...
1. Spawn the review agent per `.claude/review-gate.md` against `planning/gtm-phase-2-plan.md` → "Step 2 — `attestly-cloud` MVP Scaffold", with emphasis on the exit criteria (especially "Tampered attestation surfaces as HTTP 422" and "Staging deployment live at a real URL").
2. Capture the review agent's structured report.
3. Open a draft PR per `.claude/pull-requests.md` (target `main`, title `feat(cloud): gtm phase 2 attestly-cloud MVP scaffold`).
...
# Local stack up
cd services/attestly-cloud && docker compose up -d
# → postgres, minio, attestly-cloud-api all healthy within 30s
...
# Staging deployment
flyctl deploy --config services/attestly-cloud/fly.toml
# → https://attestly-cloud-staging.fly.dev returns 200 on GET /healthz
```
...
You are implementing Step 3 of GTM Phase 2 of Attestly. The full specification is in `planning/gtm-phase-2-plan.md` under "Step 3 — LangGraph SDK v0.1". Read that section carefully before writing any code.

### Context

Steps 1 and 2 are complete and merged:

- `crates/attestly-python/` exposes the Rust signing path to Python as the `attestly` module on TestPyPI (Step 1). Byte-identical signatures are guaranteed across Rust and Python.
- `services/attestly-cloud/` runs locally via `docker compose up` and accepts attestations at `POST /v1/attestations` (Step 2). The HTTP contract is locked at `services/attestly-cloud/API.md`.
...
Your job is to ship the adoption wedge: a one-line `pip install attestly-langgraph` that wraps any LangGraph `StateGraph` and emits a signed attestation per node execution. This SDK is the marketing budget — every team that installs it is top-of-funnel for Step 6's design-partner conversion.

This step is **Python only.** Step 4 (pricing/billing) ships in parallel in `services/attestly-cloud/src/billing.rs` and `tools/landing-page/pricing.html` — no merge collisions.

### Your Task

1. **Create `python/attestly-langgraph/`** as a new Python package. Build backend: `hatchling` or `setuptools` (your call — favour `hatchling` for simplicity). Package name: `attestly-langgraph`; import path: `attestly.langgraph`. Depend on `attestly-core-py` (Step 1's TestPyPI package) for signing.
...
3. **One-line integration mechanics.** Wrapping a graph attaches a LangGraph callback (`langgraph` exposes a callback/listener interface — use the documented one; if multiple exist, pick the one most stable across LangGraph 0.2.x). On every node completion, the callback:
   ...
   - Signs via `attestly.sign_attestation(payload, identity)`
...
4. **Sinks (pluggable `Sink` protocol):**
   ...
   - `CloudSink(api_key: str, endpoint: str = "https://api.attestly.xyz")` — POSTs to `attestly-cloud` per `services/attestly-cloud/API.md`. Retry on 5xx with exponential backoff (3 attempts max). Surface 422 (signature invalid) as a loud Python exception — that indicates the SDK is broken, not the network.
...
8. **Example** at `python/attestly-langgraph/examples/kyc_graph.py` — a LangGraph version of the KYC demo from GTM Phase 1 Step 2, demonstrating the one-line wrap and end-to-end attestation flow. Three scenarios (Approve, Flag, Tampered) per the Phase 1 demo. The tampered scenario uses dual-agent mode and surfaces the disagreement.

9. **Performance budget:** signing adds <10ms per node on a modern laptop. Add a microbenchmark in `python/attestly-langgraph/tests/bench_signing.py` (using `pytest-benchmark` or hand-rolled `time.perf_counter`). Document the measured number in the README. Ed25519 is cheap, but measure — JSON canonicalisation can be the slow path.

10. **README** at `python/attestly-langgraph/README.md`: install, quickstart (the 5-line snippet from Step 5's quickstart works here), sink configuration, identity management, dual-agent mode, troubleshooting (especially "my attestations aren't appearing in attestly-cloud" — most likely `CloudSink` 422 or wrong API key).

11. **Tests:**
    - Unit tests for each sink (file write round-trip; cloud sink against a mocked HTTP server; callable invocation)
    - Integration test: build a 3-node graph, wrap it, run it, assert 3 attestations written to a `FileSink` and that each one verifies against the audit viewer's verification path (call into `attestly-core-py` for verification)
    - Byte-identical test: an attestation produced by `attestly-langgraph` and an attestation produced by the Rust `kyc_receipts` example with the same input must have identical signing-payload bytes
...
None — this is the last step on the SDK side. Step 4 lands the pricing page and Stripe wiring in parallel; the only shared surface is the `CloudSink` ↔ `POST /v1/attestations` contract, which is locked by `services/attestly-cloud/API.md` from Step 2.

### Do Not Touch

- `crates/attestly-python/` — Step 1's domain (depend on the published TestPyPI package, do not modify the source in this branch)
- `services/attestly-cloud/` other than reading `API.md` — Step 2's domain
- `tools/landing-page/pricing.html` — Step 4's domain
...
# Install the SDK
pip install --index-url https://test.pypi.org/simple/ attestly-langgraph
# → installs along with attestly-core-py
...
# Cloud sink against local attestly-cloud
docker compose -f services/attestly-cloud/docker-compose.yml up -d
ATTESTLY_API_KEY=test python python/attestly-langgraph/examples/kyc_graph.py --sink cloud
curl -H "x-api-key: test" 'http://localhost:8080/v1/attestations?limit=10'
# → attestations appear in attestly-cloud search results

# Performance budget
pytest python/attestly-langgraph/tests/bench_signing.py -v
...
You are implementing Step 4 of GTM Phase 2 of Attestly. The full specification is in `planning/gtm-phase-2-plan.md` under "Step 4 — Pricing, Billing, and Public Pricing Page". Read that section carefully before writing any code.

### Context

Steps 1 and 2 are complete and merged:

- `services/attestly-cloud/` is running at a staging URL with the API key model in place (Step 2). A `usage` table records daily attestation counts per account. A Stripe webhook stub exists at `services/attestly-cloud/src/billing.rs` returning 501.
...
This step is **service + landing page only.** Step 3 (LangGraph SDK) ships in parallel in `python/attestly-langgraph/` — no merge collisions; the only shared surface is the locked `CloudSink` ↔ `POST /v1/attestations` contract.
...
3. **Stripe integration in `attestly-cloud`** (`services/attestly-cloud/src/billing.rs`):
   ...
   - **Customer portal** — accessible from the `attestly-cloud` account dashboard. Plan changes, invoices, and cancellation flow through Stripe's hosted portal (don't build a custom billing UI).
...
6. **Account dashboard.** Add to the `attestly-cloud` web viewer: current plan, API key management (rotate, revoke), usage chart (attestations per day for the last 30 days), Stripe portal link, export button.
...
- `crates/attestly-python/` — Step 1's domain
- `python/attestly-langgraph/` — Step 3's domain
- `services/attestly-cloud/src/` other than `billing.rs` and the dashboard — keep the diff scoped to billing concerns
...
docker compose exec attestly-cloud /usr/local/bin/sweeper run-once
...
You are implementing Step 5 of GTM Phase 2 of Attestly. The full specification is in `planning/gtm-phase-2-plan.md` under "Step 5 — Quickstart, Docs, and Conversion Telemetry". Read that section carefully before writing any code.

### Context

All prior Phase 2 steps are merged:

- Step 1: `attestly-core-py` on TestPyPI (Rust signing exposed to Python)
- Step 2: `services/attestly-cloud/` live at a staging URL with API key and Stripe-stub plumbing
- Step 3: `attestly-langgraph` on TestPyPI (the one-line LangGraph wrapper)
...
1. **Quickstart at `https://attestly.xyz/quickstart`** — a 60-second flow:
   1. Sign up (email + password, or OAuth — your call; favour email+password for simplicity; OAuth is a v0.2 polish)
   2. Display `pip install attestly-langgraph` and the API key
   ...
   The quickstart page is part of `services/attestly-cloud/web/` — server-rendered.

2. **Documentation site at `docs.attestly.xyz`.** Use a static-site generator that does not lock you into a vendor — `mkdocs` with the `material` theme is the recommended default (alternative: `docusaurus` if you prefer JS tooling). Source lives at `docs/site/` in the repo. Sections:
   ...
   - **LangGraph integration** — full SDK reference (auto-generated from `attestly-langgraph` docstrings where possible)
...
3. **Anonymous SDK telemetry** in `attestly-langgraph` (opt-out by default per the plan; if the human flags community pushback in the first 30 days, the decision to flip to opt-in is theirs, not yours):
   - Emits a daily aggregate POST to `https://telemetry.attestly.xyz/v1/usage`:
     ```json
     {"install_id": "<uuid generated on first import>",
      ...
      "os": "darwin"}
     ```
   - **No payload data. No agent IDs. No customer IDs. No user identifiers beyond the install UUID.** The install UUID is generated locally on first import and stored at `~/.config/attestly/install_id`.
   - Opt-out via `ATTESTLY_TELEMETRY=0` environment variable or `attestly.langgraph.telemetry_disable()`. Documented prominently in the SDK README's first 200 words.
   - Server endpoint in `services/attestly-cloud/` aggregates daily counts in a separate `telemetry_events` table (no PII, no joins to `accounts`).
...
5. **Root `README.md` update.** Lead the install section with the Python SDK (`pip install attestly-langgraph`), and demote the Rust prototype examples to a "Reference implementation" subsection. Keep the existing market research and prototype-plan links intact.
...
- `crates/attestly-python/` — Step 1's domain
- `services/attestly-cloud/src/` other than the quickstart route and the telemetry ingest endpoint — keep the diff scoped to docs/quickstart/telemetry
- `python/attestly-langgraph/` other than adding the telemetry module — additive only, no API changes
...
# → one POST to telemetry.attestly.xyz/v1/usage with aggregate stats

# Telemetry opt-out works
ATTESTLY_TELEMETRY=0 python python/attestly-langgraph/examples/kyc_graph.py
# → zero network calls to telemetry domain (verified via mitmproxy)

# Install ID is stable across runs but anonymous
cat ~/.config/attestly/install_id
...
head -20 README.md
# → `pip install attestly-langgraph` appears in the install section
...
GTM Phase 2 Steps 1–5 are merged. The `attestly-langgraph` SDK ships with `FileSink`, `CloudSink`, and `CallableSink`. The next step in the GTM funnel is signing Design Partner #1 — a regulated buyer already running LangGraph in production. Most such teams also use LangSmith for tracing and observability. Attestly must show up as **additive to LangSmith**, not as a competitive replacement, or the conversation never reaches the pilot stage.

Your job is to add an option that injects Attestly attestation IDs into LangSmith trace metadata, so a user looking at a LangSmith trace can click through to the corresponding Attestly attestation in `attestly-cloud`. This is small but load-bearing: it's the single most-effective procurement-conservative-buyer reassurance ("we're not asking you to rip out anything").

### Your Task

1. **Add a `langsmith_callback` option** to `attest()` in `python/attestly-langgraph/`:
   ```python
   graph = attest(graph, agent_id="my-agent-01", langsmith_callback=True)
   ```
   When enabled, the SDK detects whether the active LangGraph run has a LangSmith tracer attached (via `langsmith.run_helpers.get_current_run_tree()` or the equivalent stable API for the LangSmith version pinned by `attestly-langgraph`).
...
2. **Inject attestation metadata into LangSmith traces.** For each node execution that produces an attestation, write the attestation ID into the corresponding LangSmith run's metadata:
   ```python
   run.add_metadata({"attestly_attestation_id": attestation.id,
                     "attestly_attestation_url": f"{cloud_endpoint}/attestations/{attestation.id}"})
   ```
...
4. **Test against a real LangSmith workspace.** Create a test LangSmith account (free tier is fine) and run the `kyc_graph.py` example with `langsmith_callback=True`. Confirm:
   ...
   - The "View in LangSmith" link in the Attestly viewer opens the correct trace
   - Disabling the option produces no LangSmith calls
   Document the test workspace configuration in `python/attestly-langgraph/tests/test_langsmith_integration.py` (with credentials read from env vars; don't commit any).

5. **Documentation.** Add a "LangSmith integration" section to `docs.attestly.xyz/concepts/` and to `python/attestly-langgraph/README.md`. Frame as: "Attestly + LangSmith — provenance and observability in one trace." (The eventual joint blog post with LangChain reuses this framing; coordinate with the founder before publishing externally.)
...
- `crates/attestly-python/` — not your domain
- `services/attestly-cloud/src/` other than the viewer template update for the LangSmith link — keep the diff scoped
- `python/attestly-langgraph/` core API surface — additive only, do not change existing signatures
...
4. Post the Agent Run Report comment combining `git log main..HEAD --oneline` with the review report, plus a screenshot showing the Attestly attestation ID inside a real LangSmith trace.
...
LANGSMITH_API_KEY=ls_... python python/attestly-langgraph/examples/kyc_graph.py --langsmith
# → attestations emitted as usual
# → LangSmith traces include attestly_attestation_id metadata
# → check https://smith.langchain.com — metadata visible in trace detail view

# Reverse link in viewer
# Open https://app.attestly-cloud-staging.fly.dev (or local docker stack)
...
python python/attestly-langgraph/examples/kyc_graph.py --langsmith
# → graph runs to completion; warning log line emitted; no exception
# → Attestly attestations still emitted normally
```
