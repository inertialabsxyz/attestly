...
**Duration:** 4 weeks
**Driver:** Six-month GTM plan in [`../attestly-market-research.md`](../attestly-market-research.md), Months 1–3 (positioning → public launch → design partner hunt).
...
This plan supersedes the protocol-driven "Phase 2 of Attestly overall" sequencing in `attestly-prototype-plan.md` → "Next Steps After Week 8" for the next four weeks. The supersession is documented in [`../docs/PHASE1_REVIEW.md`](../docs/PHASE1_REVIEW.md) and [`../docs/USER_JOURNEYS.md`](../docs/USER_JOURNEYS.md).
...
4. **Compliance pre-mapping** — one document mapping Attestly attestation features to specific SR 11-7 (model risk management) clauses. Persona A sales asset and the first concrete instance of GTM §4.2 lever #5.
...
- **Hosted service (`attestly-cloud`).** Per GTM §4.1 lock #3, this is non-negotiable for revenue but is Month 3+ work. The prototype's library-and-CLI shape is sufficient for Month 2's demo video and Month 3's design-partner conversations.
...
3. New tool in `crates/attestly-agents/src/tools.rs`: `kyc_decide(request: &KycRequest) -> KycDecision`. Pure function; deterministic; no external state.
...
1. New module `crates/attestly-core/src/identity.rs` defining:
...
5. Document the on-disk format in `crates/attestly-core/src/identity.rs` module-level rustdoc — fields, JSON shape, file permissions.
...
1. New document `docs/compliance/SR_11_7.md`. Structure:
   - **Preamble:** what SR 11-7 is (US Federal Reserve guidance on model risk management), why it matters for agent-driven decisions, and Attestly's claim scope (we attest to *what the agent did*, not *whether the model is correct under SR 11-7*).
   - **Scope and limits:** explicit list of what Attestly's attestations help with and what they do not. Avoid overclaiming — see the warning below.
   - **Clause-by-clause mapping table:** each row is one specific SR 11-7 clause / subsection, the requirement it imposes, and which Attestly feature(s) help meet it. Cite the Attestly feature by file or doc reference (e.g. *"Attestly `Attestation.timestamp` + `signature` covers SR 11-7 §V.A.3 'maintain a log of model use' for agent decisions"*).
   - **Sample audit narrative:** a worked example of how a Risk Officer would use Attestly receipts to answer an SR 11-7 examination question — referencing the KYC demo's output for concreteness.
   - **What Attestly does *not* provide:** a candid section listing SR 11-7 requirements that are out of scope (model validation, ongoing monitoring of model performance, governance committee structures, etc.).
...
**This document is legal-interpretation work. It must not overclaim Attestly's coverage.** SR 11-7 covers model lifecycle governance broadly; Attestly attests to *agent execution events*, which is one strand of one section. Any clause we cite as "covered" must be honestly defensible — overclaiming damages credibility with the exact buyers we're trying to win.

The reviewing agent (Closing the Loop step) should explicitly check for overclaims and flag any clause where the Attestly feature cited is necessary-but-not-sufficient for compliance.

### Exit criteria

- [ ] `docs/compliance/SR_11_7.md` exists with all five sections populated
- [ ] Each "covered" clause cites a specific Attestly type / file / behaviour
- [ ] The "What Attestly does not provide" section is at least as substantial as the "covered" section
...
- [ ] Reviewer (review-gate) confirms no overclaim — every cited Attestly feature is verifiable in code
...
| SR 11-7 mapping overclaims coverage | Medium | High | Reviewer mandate + the "What Attestly does not provide" section gates this |
