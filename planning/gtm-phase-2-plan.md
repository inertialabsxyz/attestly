...
**Duration:** 8 weeks
**Driver:** Six-month GTM plan in [`../attestly-market-research.md`](../attestly-market-research.md), Months 3–5 (design partner hunt → ecosystem wedge → sales repeatability).
**Goal:** Convert the OSS credibility built in GTM Phase 1 into a paying design partner by shipping the LangGraph SDK (the adoption wedge) and `attestly-cloud` MVP (the paid surface), with public pricing and a free-to-paid conversion path that survives a procurement conversation at a mid-market regulated buyer.

## Context

GTM Phase 1 ([`gtm-phase-1-plan.md`](gtm-phase-1-plan.md)) makes the prototype demo-able to Sarah (compliance lead) and unblocks Marcus (platform operator). What it does **not** do is give either persona a way to *try Attestly on their own workload*, which is the gating moment between "watched the demo" and "signed a paid pilot."

The driving insight for this phase: **the OSS SDK is the marketing budget.** Every team that installs `attestly-langgraph` is top-of-funnel; conversion to paid happens at specific lifecycle moments (first auditor conversation, first multi-team deployment, first compliance deadline). Without the SDK there is no funnel — the §5 outbound motion in the market research has nothing to sequence against. With it, the §3.3 SOM math becomes operational: outbound to accounts who already produced >100k attestations against their own LangGraph agents last month.

The monetization architecture is **open-source-core, paid-hosted**, modelled on LangSmith / Helicone / Braintrust / Sentry / Posthog. The OSS SDK must be fully functional offline so that procurement-conservative buyers can adopt without a SaaS dependency on day one. The hosted service (`attestly-cloud`) earns its keep on retention, sharing, and compliance — the things a team will not build themselves.

## Scope

In scope for this phase:

1. **LangGraph SDK v0.1** — `pip install attestly-langgraph`. Wraps a `StateGraph`; every node execution emits a signed attestation to a configurable sink. PyO3 bindings to the existing Rust core (no Python rewrite of signing).
2. **`attestly-cloud` MVP** — hosted ingest endpoint, hosted viewer, share-links, search by agent/customer/time, one-year retention. Single region, single AZ. Stripe billing for the Team tier.
...
5. **LangSmith metadata integration** — optional one-line config that emits Attestly attestation IDs into LangSmith traces, so existing LangSmith users see Attestly as additive rather than competitive.
...
- **Multi-region `attestly-cloud`.** Single region (US-East) only. EU residency is a Phase 3 enterprise concern.
...
```
Week 1-2 (parallel):  Step 1 — PyO3 Bindings        Step 2 — attestly-cloud MVP scaffold
                              │                            │
                              └────────────┬───────────────┘
                                           │
...
- Step 1 (PyO3 bindings) and Step 2 (`attestly-cloud` scaffold) are independent — one is Rust + Python, the other is a hosted service. Run in parallel worktrees.
...
1. New crate `crates/attestly-python` — PyO3 bindings exposing the Attestly signing path to Python. ~200 lines of Rust.
2. Python-side API surface:
   ...
   - `attestly.Attestation` — dataclass-shaped Python object backed by Rust struct
...
5. Published to TestPyPI under `attestly-core-py` (real PyPI publish gated behind the design-partner close in Step 6).
6. README at `crates/attestly-python/README.md` documenting install, usage, and the byte-identical-signature guarantee with example.

### Exit criteria

- [ ] `pip install --index-url https://test.pypi.org/simple/ attestly-core-py` succeeds on macOS, Linux, Windows
...
## Step 2 — `attestly-cloud` MVP Scaffold

**Owner:** dispatched agent on `gtm-phase-2/attestly-cloud-scaffold`

### Deliverables

1. New directory `services/attestly-cloud/` containing the hosted-service codebase. Stack: Rust (Axum) for the API, Postgres for storage, S3 (or compatible) for attestation blob storage, deployed as a single container.
2. HTTP API surface:
...
5. Web viewer at `https://app.attestly.xyz` (placeholder domain) — same UX as the static viewer (Phase 1 Step 1) but server-backed: search, share-link generation, account dashboard. Reuse the existing static viewer's JS verification logic.
...
1. New Python package `python/attestly-langgraph/` published as `attestly-langgraph` on TestPyPI.
2. Primary API:
   ```python
...
4. Sinks (pluggable, `Sink` protocol):
   ...
   - `CloudSink(api_key: str, endpoint: str = "...")` — POST to `attestly-cloud`. Stripe billing hooks on the cloud side count attestations.
...
8. Example: `python/attestly-langgraph/examples/kyc_graph.py` — a LangGraph version of the KYC demo from GTM Phase 1 Step 2, demonstrating the one-line wrap and end-to-end attestation flow.
9. Documentation at `python/attestly-langgraph/README.md`: install, quickstart, sink configuration, identity management, dual-agent mode, troubleshooting.

### Exit criteria

- [ ] `pip install attestly-langgraph` installs cleanly on macOS, Linux, Windows
...
- [ ] `CloudSink` successfully POSTs to a local `attestly-cloud` instance and the attestation appears in search results
...
2. **Stripe integration** in `attestly-cloud`:
...
3. **Account model**: an `attestly-cloud` account holds one or more API keys. Each API key is scoped to a project. Attestation usage is aggregated per account for billing.
...
1. **Quickstart at `https://attestly.xyz/quickstart`** — 60-second flow:
...
2. **Documentation site** at `docs.attestly.xyz`. Sections:
...
3. **Anonymous SDK telemetry** (opt-in by default, off-by-config):
   ...
   - Documented prominently in the SDK README with an opt-out instruction (`ATTESTLY_TELEMETRY=0`)
...
- [ ] Telemetry produces aggregate stats visible in a dashboard for Attestly team
- [ ] Opt-out works: `ATTESTLY_TELEMETRY=0` produces no network calls (verified by mitmproxy or equivalent)
...
4. **LangSmith metadata integration** (engineering deliverable, ~2 days):
   ...
   - Each LangGraph node's LangSmith trace gets an `attestly_attestation_id` field pointing to the attestation
   - The `attestly-cloud` viewer can render a "View in LangSmith" link if the trace ID is present
   - Joint blog post pitch to LangChain ("Attestly + LangSmith: provenance and observability in one trace") — sent in Phase 3 Month 5 but the integration ships here so the post can reference live functionality
...
- [ ] `pip install attestly-langgraph` works against the real PyPI (not TestPyPI)
- [ ] `attestly-cloud` is live at its production URL with at least 3 internal-test accounts and the design-partner account
...
| `attestly-cloud` scope expands into a full multi-region service | High | Medium | Hard rule: single region, single AZ, no SLA promise on the public pricing page. Multi-region is Phase 3+. |
...
2. Their engineering team installed `attestly-langgraph` on a staging branch the next day; 4,200 attestations produced overnight on `FileSink`
...
The thing the previous plan iterations did not name explicitly: **the OSS SDK is the marketing budget.** Every team that installs `attestly-langgraph` is top-of-funnel; conversion happens at the lifecycle moments above (first auditor conversation, first multi-team deployment, first compliance deadline, first cross-org dispute, first scale wall). Without the SDK there is no funnel; with it, the §5 outbound motion in the market research becomes "outbound to accounts already producing >10k attestations / day against their LangGraph agents."
